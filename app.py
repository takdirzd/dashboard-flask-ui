...
def index():
    ...
    stats = {
        ...
        'rev_table' :   df2.groupby(by=['Category','App']).mean().sort_values(by=['Reviews'],ascending=False).reset_index().head(10)[['Category', 'App', 'Reviews', 'Rating']].to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
            }   
    ...
    content2 = pd.crosstab(index=df2['Category'],
                       columns=df2['Type']).sort_values(by='Free',ascending=False).head(5)
    content2.plot(kind ='bar', rot = 0,figsize=(5,5))
    plt.savefig('Category_Type.png',bbox_inches="tight")
